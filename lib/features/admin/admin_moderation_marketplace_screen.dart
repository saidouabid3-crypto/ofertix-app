...
class AdminModerationMarketplaceScreen extends StatefulWidget {
  const AdminModerationMarketplaceScreen({super.key});

  @override
  State<AdminModerationMarketplaceScreen> createState() => _AdminModerationMarketplaceScreenState();
}

class _AdminModerationMarketplaceScreenState extends State<AdminModerationMarketplaceScreen> {
  static const _filters = ['active', 'pending', 'hidden', 'rejected'];

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AdminProvider>().loadMarketplace();
    });
  }
      ...
      children: [
        Container(
          ...
          child: ListView(
            ...
            children: _filters.map((f) {
              final selected = provider.marketplaceStatusFilter == f;
              return GestureDetector(
                onTap: () => provider.loadMarketplace(status: f),
                child: Container(
        ...
        Expanded(
          child: provider.isLoadingMarketplace
              ...
              : provider.marketplaceError != null
                  ? _ErrorRetry(msg: provider.marketplaceError!, onRetry: provider.loadMarketplace)
                  : provider.marketplaceItems.isEmpty
                      ? _EmptyState('admin.noData'.tr())
                      : RefreshIndicator(
                          color: AppColors.orange,
                          onRefresh: provider.loadMarketplace,
                          child: ListView.builder(
                            padding: const EdgeInsets.all(16),
                            itemCount: provider.marketplaceItems.length,
                            itemBuilder: (context, i) => _MarketplaceCard(
                              item: provider.marketplaceItems[i],
                              onAction: (action, reason) async {
                                final ok = await provider.moderateMarketplaceItem(
                                  provider.marketplaceItems[i].id,
                                  action,
                                  reason: reason,
                                );
                                if (context.mounted) {
...
class _MarketplaceCard extends StatelessWidget {
  const _MarketplaceCard({required this.item, required this.onAction});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                  ...
                  children: [
                    ...
                    Row(children: [
                      CircleAvatar(
                        ...
                        backgroundImage: item.creatorAvatarUrl?.startsWith('http') == true
                            ? NetworkImage(item.creatorAvatarUrl!)
                            : null,
                        child: item.creatorAvatarUrl?.startsWith('http') != true
          ...
          _ActionRow(status: item.status, onAction: onAction),
        ],
      ),
    );
  }
  ...
  Widget _placeholder() => Container(
        ...
        child: const Icon(Icons.storefront_rounded, color: AppColors.gray),
      );
}
    ...
    return Wrap(
      ...
      children: [
        if (status != 'approved' && status != 'active')
          _Btn(label: 'admin.approve'.tr(), color: AppColors.green, icon: Icons.check_rounded, onTap: () => onAction('approve', null)),
        if (status != 'rejected')
          _Btn(label: 'admin.reject'.tr(), color: AppColors.red, icon: Icons.close_rounded, onTap: () => _promptReason(context, 'reject')),
        if (status != 'hidden')
          _Btn(label: 'admin.hide'.tr(), color: AppColors.gray, icon: Icons.visibility_off_rounded, onTap: () => _promptReason(context, 'hide')),
        if (status == 'hidden' || status == 'rejected')
          _Btn(label: 'admin.restore'.tr(), color: Colors.blue, icon: Icons.restore_rounded, onTap: () => onAction('restore', null)),
      ],
    );
  }
